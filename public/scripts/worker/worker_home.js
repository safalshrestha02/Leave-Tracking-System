...
const pendingApprovedLeaves = document.querySelector(
  ".pending-and-approved-container"
);
...
const fetchTotalLeaves = async () => {
  try {
    ...
    leavesRemaining.textContent =
      parseInt(totalLeaveDays.textContent) - parseInt(leavesTaken.textContent);
  } catch (error) {
    console.log(error);
  }
};
...
const activeLeavesDisplay = async () => {
  try {
    ...
    const activeLeaves = totalLeaves.filter((leave) => {
      ...
      const endDateLeave = `${leave.endDate.slice(0, 4)}${leave.endDate.slice(
        5,
        7
      )}${leave.endDate.slice(8, 10)}`;
        ...
        let ihtml = `
          ...
          .replaceAll("-", "/")}</span> <span> ${leave.leaveDays} days</span>
                ...
                <p class="leave-status">
                  <i class="fa-solid fa-circle green"></i><span class="capitalize-input">${
                    leave.approveState
                  }</span>
                </p>
            `;
    ...
    totalPendingLeaves.forEach((leave) => {
      let ihtml = `
            <div class="worker-leaves">
                <div class="leave-type-container">
                    ...
                    <div class="delete-leave" id='${leave._id}'>
                        <i class="fa-solid fa-trash" onClick='handleDeleteIcon("${
                          leave._id
                        }")'></i>
                    </div>
                </div>
        ...
        .replaceAll("-", "/")}</span> <span> ${leave.leaveDays} days</span>
                ...
                <p><i class="fa-solid fa-circle orange"></i><span class="capitalize-input leave-status">${
                  leave.approveState
                }</span></p>
            </div>
            `;
