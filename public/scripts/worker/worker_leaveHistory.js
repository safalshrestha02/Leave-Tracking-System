  ...
  try {
    ...
    if (workersLeaveHistory.length > 0) {
      ...
      workersLeaveHistory.map((data) => {
        ...
        if (
          data.approveState == "approved" &&
          parsedCurrentDate > parsedendDate
        ) {
          pastArray.push(data);
        }
      ...
      let sortedUpcoming = () => {
        let sortedUpcomingArray = upcomingArray.sort((a, b) => {
          return (
            Date.parse(a.startDate.slice(0, 10)) -
            Date.parse(b.startDate.slice(0, 10))
          );
        });
      ...
      let sortedPast = () => {
        let sortedPastArray = pastArray.sort((a, b) => {
          return (
            Date.parse(a.endDate.slice(0, 10)) -
            Date.parse(b.endDate.slice(0, 10))
          );
        });
...
const confirmDelete = (monId, index) => {
  ...
  const confirmContainer = document.querySelectorAll(
    ".confirm-leave-delete-container"
  );
