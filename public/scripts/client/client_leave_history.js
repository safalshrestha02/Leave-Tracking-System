...
const leaveHistoryContainer = document.querySelector(
  ".client-leave-history-container"
);
...
const rejectedLeaveContainer = document.querySelector(
  ".rejected-leave-history-container"
);
const upcomingLeaveContainer = document.querySelector(
  ".upcoming-leave-history-container"
);
const pastLeaveContainer = document.querySelector(
  ".past-leave-history-container"
);
...
const checkWorkers = async () => {
  ...
  if (totalCompanyWorkers.length === 0) {
    leaveHistoryContainer.innerHTML =
      '<p class="empty-workers">You have no any workers. Add some to see their past leaves...</p>';
  }
};
checkWorkers();
...
const leaveHistory = async () => {
  ...
  const getLeaveHistory = async () => {
    ...
    const sortedLeaveHistory = approvedAndRejectedLeaves.sort(
      (leaves1, leaves2) => {
    ...
    fetchLeaveHistory = async () => {
      ...
      if (
        userChoiceApprovedLeaves.length == 0 &&
        userChoiceRejectedLeaves.length > 0
      ) {
        leaveHistoryContainer.innerHTML =
          '<p class = "no-past-leaves">The requested user has no any approved leaves records but has rejected leaves records.</p>';
      }

      if (upcomingApprovedLeaves.length > 0) {
        upcomingLeaves.classList.add("show-div");
        upcomingApprovedLeaves.forEach((leave) => {
          const { typeOfLeave, startDate, endDate, leaveDays, approveState } =
            leave;
            ...
            .replaceAll("-", "/")}</span>
                            ...
                            <div class="client-leave-history-leave-status">
                                <p>
                                    ...
                                    <span class="history-leave-status">${capitalize(
                                      approveState
                                    )} </span>
                                </p>
                            </div>
    ...
    if (userChoiceLeaveHistory.length == 0) {
      ...
      const blankArr = [];
      leaveHistoryContainer.innerHTML =
        '<p class = "no-past-leaves">The requested user has no any leaves records.</p>';
      return blankArr;
    } else {
      fetchLeaveHistory();
    }
  ...
  approvedLeavesFilter.addEventListener("click", async () => {
    try {
      ...
      leaveHistoryContainer.innerHTML =
        '<p class = "no-past-leaves">The requested user has no any approved leaves records.</p>';
    }
  });
  ...
  rejectedLeavesFilter.addEventListener("click", async () => {
    ...
    if (userChoiceRejectedLeaves.length == 0) {
      leaveHistoryContainer.innerHTML =
        '<p class = "no-past-leaves">The requested user has no any rejected leaves records.</p>';
    }
